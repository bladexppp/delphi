object FrmPesquisaCliente: TFrmPesquisaCliente
  Left = 0
  Top = 0
  BorderIcons = [biSystemMenu]
  BorderStyle = bsSingle
  Caption = 'Pesquisa De Cliente'
  ClientHeight = 370
  ClientWidth = 636
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poDesktopCenter
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object PngFiltros: TPanel
    Left = 0
    Top = 0
    Width = 636
    Height = 93
    Align = alTop
    Color = clTeal
    ParentBackground = False
    TabOrder = 0
    object Label1: TLabel
      Left = 14
      Top = 6
      Width = 32
      Height = 13
      Caption = 'Nome'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object txtPesquisa: TEdit
      Left = 14
      Top = 22
      Width = 511
      Height = 21
      CharCase = ecUpperCase
      TabOrder = 0
      OnKeyDown = txtPesquisaKeyDown
    end
    object cmdProcurar: TBitBtn
      Left = 539
      Top = 20
      Width = 80
      Height = 25
      Caption = '&Procurar'
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      NumGlyphs = 2
      TabOrder = 1
      OnClick = cmdProcurarClick
    end
    object GroupBox1: TGroupBox
      Left = 14
      Top = 43
      Width = 185
      Height = 46
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      TabOrder = 2
      object Label2: TLabel
        Left = 12
        Top = 3
        Width = 81
        Height = 13
        Caption = 'Procurar em'
        Font.Charset = ANSI_CHARSET
        Font.Color = clWhite
        Font.Height = -11
        Font.Name = 'Verdana'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object rbInicio: TRadioButton
        Left = 12
        Top = 20
        Width = 113
        Height = 17
        Caption = 'In'#237'cio'
        TabOrder = 0
      end
      object rbMeio: TRadioButton
        Left = 64
        Top = 20
        Width = 113
        Height = 17
        Caption = 'Qualquer Parte'
        Checked = True
        TabOrder = 1
        TabStop = True
      end
    end
  end
  object pngBotes: TPanel
    Left = 0
    Top = 318
    Width = 636
    Height = 52
    Align = alBottom
    Color = clTeal
    ParentBackground = False
    TabOrder = 1
    object DBNavigator1: TDBNavigator
      Left = 8
      Top = 19
      Width = 344
      Height = 25
      DataSource = dsCliente
      VisibleButtons = [nbFirst, nbPrior, nbNext, nbLast]
      TabOrder = 0
    end
    object cmdOk: TBitBtn
      Left = 469
      Top = 19
      Width = 75
      Height = 25
      Caption = 'OK'
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      NumGlyphs = 2
      TabOrder = 1
      OnClick = cmdOkClick
    end
    object cmdCancela: TBitBtn
      Left = 549
      Top = 19
      Width = 75
      Height = 25
      Caption = 'Cancelar'
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      NumGlyphs = 2
      TabOrder = 2
      OnClick = cmdCancelaClick
    end
  end
  object Panel3: TPanel
    Left = 0
    Top = 93
    Width = 17
    Height = 225
    Align = alLeft
    Color = 6570553
    ParentBackground = False
    TabOrder = 2
    ExplicitTop = 61
    ExplicitHeight = 257
  end
  object grdPesquisa: TDBGrid
    Left = 17
    Top = 93
    Width = 619
    Height = 225
    Align = alClient
    DataSource = dsCliente
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
    ReadOnly = True
    TabOrder = 3
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'Tahoma'
    TitleFont.Style = []
    OnDblClick = cmdOkClick
    OnKeyDown = grdPesquisaKeyDown
    Columns = <
      item
        Expanded = False
        FieldName = 'COD_CLIENTE'
        Title.Caption = 'C'#243'digo'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'NOME'
        Title.Caption = 'Nome'
        Width = 416
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'CPF'
        Width = 94
        Visible = True
      end>
  end
  object Cliente: TFDQuery
    Connection = DMDados.Bdados
    SQL.Strings = (
      'select *'
      '  from cliente'
      '   where nome like :WNOME'
      '   AND FLG_ATIVO ='#39'S'#39
      '   order by nome'
      ''
      '')
    Left = 460
    Top = 140
    ParamData = <
      item
        Name = 'WNOME'
        DataType = ftString
        ParamType = ptInput
        Size = 100
        Value = Null
      end>
    object ClienteCOD_CLIENTE: TIntegerField
      FieldName = 'COD_CLIENTE'
      Origin = 'COD_CLIENTE'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object ClienteNOME: TStringField
      FieldName = 'NOME'
      Origin = 'NOME'
      Required = True
      Size = 100
    end
    object ClienteCPF: TStringField
      FieldName = 'CPF'
      Origin = 'CPF'
      Required = True
      EditMask = '000.000.000-00;0;_'
      Size = 11
    end
    object ClienteDATA_NASCIMENTO: TDateField
      FieldName = 'DATA_NASCIMENTO'
      Origin = 'DATA_NASCIMENTO'
    end
    object ClienteEMAIL: TStringField
      FieldName = 'EMAIL'
      Origin = 'EMAIL'
      Size = 200
    end
    object ClienteENDERECO: TStringField
      FieldName = 'ENDERECO'
      Origin = 'ENDERECO'
      Size = 200
    end
    object ClienteNUMERO: TStringField
      FieldName = 'NUMERO'
      Origin = 'NUMERO'
      Size = 10
    end
    object ClienteBAIRRO: TStringField
      FieldName = 'BAIRRO'
      Origin = 'BAIRRO'
      Size = 100
    end
    object ClienteSEXO: TStringField
      FieldName = 'SEXO'
      Origin = 'SEXO'
      FixedChar = True
      Size = 1
    end
    object ClienteTELEFONE: TStringField
      FieldName = 'TELEFONE'
      Origin = 'TELEFONE'
      Size = 21
    end
    object ClienteFLG_ATIVO: TStringField
      FieldName = 'FLG_ATIVO'
      Origin = 'FLG_ATIVO'
      FixedChar = True
      Size = 1
    end
    object ClienteCIDADE: TStringField
      FieldName = 'CIDADE'
      Origin = 'CIDADE'
      Size = 100
    end
    object ClienteCEP: TStringField
      FieldName = 'CEP'
      Origin = 'CEP'
      Size = 10
    end
  end
  object dsCliente: TDataSource
    DataSet = Cliente
    Left = 512
    Top = 140
  end
end
