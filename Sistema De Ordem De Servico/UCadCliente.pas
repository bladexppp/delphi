...
begin
  if MessageBox(Application.Handle, 'Este Cliente Está Ativo' + #013
   + 'Deseja Inativar ?',
  'Confirma inativação', MB_YESNO + MB_ICONQUESTION) = MRYES then
     begin
      Cliente.Edit;
...
begin
  if Cliente.FieldByName('FLG_ATIVO').AsString = 'N' then
    begin
      Mensagem('Imposivel Alterar! Cliente Está Inativo.');
      cmdNovo.SetFocus;
      abort;
    end;
...
begin
  ...
  if Cliente.FieldByName('CPF').text='' then
    ...
    begin
      Mensagem('Obrigatório Informar O Telefone');
...
begin
    if Cliente.FieldByName('FLG_ATIVO').AsString = 'N' then
    begin
    Mensagem('Imposivel Deletar! Cliente Está Inativo.');
    cmdNovo.SetFocus;
    abort;
    end;
